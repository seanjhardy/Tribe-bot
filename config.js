...
const config = {
  ...
  permLevels: [
    ...
    {
      level: 1,
      name: "Tribe Moderator",

      check: (message) => {
        try {
          return message.member.roles.cache.has(process.env.tribeModRole); //Tribe mod role id goes here
        } catch (e) {
          return false;
        }
      },
...

    {
      ...
      check: (message) => {
        try {
          return message.member.roles.cache.has(process.env.chiefRole); //Tribe Chief role id goes here
        } catch (e) {
          return false;
        }
      },
